...
namespace OnlineMaps
{
    ...
    /// </summary>
    [AddComponentMenu("Infinity Code/Online Maps/Third-Party Connectors/uPano Connector")]
    [ThirdPartyPlugin("uPano Connector", typeof(ControlBase))]
    public class PanoConnector : MonoBehaviour
    {
        ...
        /// <summary>
        /// Use rotation from Camera Orbit
        /// </summary>
        ...
        private bool CheckOverlayColor(GeoPoint location)
        {
            TilePoint t = location.ToTile(map);
            Tile tile = map.tileManager.GetTile(map.view.intZoom, (int) t.x, (int) t.y);
            if (tile == null) return false;
            while (tile.status != TileStatus.loaded)
            {
                tile = tile.parent;
                if (tile == null) return false;
            }
            ...
            else if (overlayLayer == OverlayLayer.traffic) overlay = (tile as RasterTile).trafficTexture;

            if (overlay == null) return false;

            int px = (int) Math.Round((t.x - (int) t.x) * 255);
            int py = 255 - (int) Math.Round((t.y - (int) t.y) * 255);
        ...
        private void LoadNextPanorama(IInteractiveElement element)
        {
        ...
        private void OnDisable()
        {
            if (map != null)
            {
                ...
                {
                    foreach (Tile tile in map.tileManager.tiles) SetOverlay(tile as RasterTile, null);
                }
                map.Redraw();
            }
        }
        ...
        private void OnEnable()
        {
            ...
            {
                Debug.LogWarning("Please specify Pano Connector/Google Api Key!!!");
                Destroy(this);
                return;
            }
            ...
            if (map == null)
            {
                ...
                if (map == null) map = Compatibility.FindObjectOfType<Map>();
                if (map == null)
                {
            ...
            foreach (KeyValuePair<ulong, Texture2D> pair in overlays)
            {
                ...
                SetOverlay(map.tileManager.dTiles[pair.Key] as RasterTile, pair.Value);
            }
        ...
        private void OnGoogleStreetViewSuccess(GoogleStreetViewRequest request)
        {
            ...
            if (panoRenderer == null)
            {
                ...
                if (useRotation && CameraOrbit.instance != null) panoRenderer.pano.pan = CameraOrbit.instance.rotation.y;
        ...
        {
            if (!enabled) return;
            if (map.view.zoom < minZoom) return;
            
            GeoPoint p = map.control.ScreenToLocation();

            if (checkOverlayColor && !CheckOverlayColor(p)) return;
            ...
            currentRequest = new GoogleStreetViewRequest(googleApiKey, p.x, p.y, zoom);
            currentRequest.OnComplete += OnGoogleStreetViewSuccess;
            this.zoom = zoom;
        }
        ...
        private void OnTileDisposed(Tile tile)
        {
            if (!overlays.ContainsKey(tile.key)) return;
            if (overlays[tile.key] != null) Utils.Destroy(overlays[tile.key]);
            overlays.Remove(tile.key);
        }

        private void SetOverlay(RasterTile tile, Texture2D texture)
        {
        ...
        private void Start()
        {
            ...
            TileManager.OnTileLoaded += StartDownloadOverlay;
        ...
        private void StartDownloadOverlay(Tile tile)
        {
            ...
            {
                if (tile.status == TileStatus.disposed || map == null) return;
                ...
                if (enabled)
                {
                    SetOverlay(tile as RasterTile, texture);
                    map.Redraw();
                }
            };
        }
        ...
        private void Update()
        {
            if (InputManager.GetKeyDown(closeByKeyCode))
            {
                Close();
            }
        }
